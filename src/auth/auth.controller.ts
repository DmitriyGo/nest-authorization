...
export class AuthController {
  ...
  @ApiBadRequestResponse({ description: "Can't register user" })
  ...
  async register(@Body() dto: RegisterDto) {
    ...
    if (!user) {
      throw new BadRequestException(`Can't register user ${JSON.stringify(dto)}`);
    }
  ...
  async login(@Body() dto: LoginDto, @Res() res: Response, @UserAgent() agent: string) {
    ...
    if (!tokens) {
      throw new BadRequestException(`Can't login with ${JSON.stringify(dto)}`);
    }
